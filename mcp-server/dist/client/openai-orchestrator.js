 ...
 */
async function sendJsonRpcRequest(postUrl, method, params) {
 ...
 */
function translateMcpToolsToOpenAi(mcpTools) {
 ...
 */
function simulateOpenAiResponse(query, messages, loopCount) {
    ...
    if (q.includes("paid maintenance") || q.includes("who has not paid")) {
        if (loopCount === 1) {
            ...
            const names = bills.map((b) => `${b.userId?.firstName || "Resident"} ${b.userId?.lastName || ""}`).join(", ");
            return {
                role: "assistant",
 ...
 */
async function runAgent(query, openaiTools, postUrl) {
    ...
    while (loopCount < maxLoops) {
        ...
        if (response.tool_calls && response.tool_calls.length > 0) {
            console.log(`[Orchestrator Loop] GPT-4 requested tool executions: ${response.tool_calls.map((tc) => tc.function.name).join(', ')}`);
                    ...
                    if (dataLine) {
                        ...
                        setTimeout(async () => {
                            try {
                                ...
                                const toolMetadata = await sendJsonRpcRequest(postUrl, "tools/list");
