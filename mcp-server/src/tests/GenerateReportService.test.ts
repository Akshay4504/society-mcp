...
export async function runGenerateReportServiceTests() {
  ...
  const mockPaymentRepo: IPaymentRepository = {
    findBills: async () => [],
    findPayments: async () => [
      { status: 'Success', amountPaid: 3200 } as any,
      { status: 'Pending', amountPaid: 1500 } as any
    ],
  ...
  const mockResidentRepo: IResidentRepository = {
    ...
    findById: async () => null,
    findAll: async () => [
      { occupancyType: 'Owner', status: 'Active' } as any,
      { occupancyType: 'Tenant', status: 'Active' } as any
    ]
  };
